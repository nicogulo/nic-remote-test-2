...
class Rangking extends MY_Controller
{
    ...
    {
        $pelatihan = $this->MPelatihan->getAll();
         ...
         */
        $table1 = $this->initialTableSAW($pelatihan);
    ...
    private function initialTableSAW($pelatihan)
    {
        $nilai = $this->MNilai->getNilaiPelatihan();

        $dataInput = array();
        $no = 0;
        foreach ($pelatihan as $item => $itemPelatihan) {
            foreach ($nilai as $index => $itemNilai) {
                if ($itemPelatihan->kdPelatihan == $itemNilai->kdPelatihan) {
                    $dataInput[$no]['pelatihan'] = $itemPelatihan->pelatihan;
